def depthFirstSearch(open, closed , start, goal):
  ...
  while (not not open):
    ...
    if (X == goal):
        ...
        X_children = ([X.get_next_states(i)[j] for i in range(1, (start.size**2) +1) for j in range(len(X.get_next_states(i)))])
        ...
        for children in X_children:
          ...
          if (not children.isIn(open) and not children.isIn(closed)):
...
def deepIterating(start, goal, max_depth):
  ...
  for i in range(max_depth):
    ...
    open = []
    closed = []
    result = dfs_limited(open,closed, start , goal, i)
    print(i, "RESULT: ",  result)
    if ((not not result) and result == goal):
      break
...
def dfs_limited(open, closed , start, goal, maxiter):
  open.append(start)
